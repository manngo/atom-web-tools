...
export default {
	...
	//	Register commands
		this.subscriptions.add(atom.commands.add('atom-workspace', {
			...
			'web-tools:html:html5': () =>				this.html5(),
			...
			'web-tools:misc:getPath': () =>				this.getPath(),
			'web-tools:misc:copyProfile': () =>			this.copyProfile(),
			'web-tools:misc:replace': () =>				this.replace(),

			'web-tools:open:init.js': () =>				this.open('~/.atom/init.js'),
			'web-tools:open:keymap.cson': () =>			this.open('~/.atom/keymap.cson'),
			'web-tools:open:hosts': () =>				this.open('!hosts'),
			'web-tools:open:httpd.conf': () =>			this.open('!httpd.conf'),
			'web-tools:open:vhosts.conf': () =>			this.open('!vhosts.conf'),
			'web-tools:open:php.ini': () =>				this.open('!php.ini'),
...
//			fs.writeFile(toolFile,`<form id="web-tools>\n${this.form.innerHTML}\n</form>\n\n`,function(data) { });
	...
	about() {
		...
		var modalPanel;
		console.log('About …');
		var html='<h2>About HTML Tools 29</h2><p>Mark Simon<br>See <a href="http://internotes.site">http://internotes.site</a> for more information …</p>';
...
//console.log(JSON.stringify(options))
	...
	entify() {
		var [editor,selection]=this.init(`Entify`);
		if(!editor) return;

		var form=`Options\n\t[]quotes0=1\tAlso Entify Quotes`;
	...
	html5() {
		var [editor,selection]=this.init(`Convert to HTML5`);
		if(!editor) return;
		var text=editor.getText(); if(!text) return;

		var form=`
Options
	[]doctype+=1	HTML5 Doctype
	[]lang+=1		Simple lang Attribute
	[]charset+=1	Modern Charset
	[]void+=1		Remove Trailing Slashes
	[]minimised+=1	Simplify Minified Attributes`;
		this.doit('html5',form,function(options) {
			editor.setText(WebTools.makeHTML5(text,options));
		});
	},
	sample() {
		var fs=require('fs');
...
console.log('Copy Profile');
		...
		var profile = `${homedir}/.atom`;
		fs.readdir(`${__dirname}/../copy/profile`, (error, files) => {
			if (error) console.log(error);
			else {
				files.forEach(file => {
					fs.copyFile(`${__dirname}/../copy/profile/${file}`,`${homedir}/.atom/${file}`, (error) => {
//					fs.copyFile(`${__dirname}/../copy/profile/${file}`,`${homedir}/atom/${file}`, (error) => {
	...
	replace() {
		var [editor,selection]=this.init(`Replace …`);
		if(!editor) return;
		var form,parms;
		form=`
Find & Replace
	...
	replace:	Replace…
`;
		this.doit('replace',form,function(options) {
//			editor.setText(WebTools.makeHTML5(text,options));
		},function() {
			form['find'].appendChild();
		});
	},
	insert() {
